    ...
    if(recovery)
      {
        ...
        while(m_queue.GetFirst (dst, queueEntry)) 
          {
            ...
            if (!tHeader.IsValid ())
              {
                ...
                m_queue.Dequeue(dst,queueEntry);  // drop invalid packet from the queue
				return false;     
              }
            if (tHeader.Get () == GPSRTYPE_POS)
              {
            ...
            if(!RecoveryMode(dst, p, ucb, header))
				return false; //recovery fails; all packets having the same dst will fail, so directly return
			else 
				m_queue.Dequeue(dst,queueEntry);  //recovery succeeds
...
RoutingProtocol::RecoveryMode(Ipv4Address dst, Ptr<Packet> p, UnicastForwardCallback ucb, Ipv4Header header){
  Ptr<Packet> cp = p->Copy (); //keep p so that it can be put back into the queue unmodified
  Vector Position;
  Vector previousHop;
  uint32_t updated;
  uint64_t positionX;
  ...
  cp->AddHeader (posHeader);
  cp->AddHeader (tHeader);
                             ...
                             UnicastForwardCallback ucb, ErrorCallback ecb)
{
  ...
  if(inRec){
    ...
    if(!RecoveryMode (dst, p, ucb, header)) 
		DeferredRouteOutput (p, header, ucb, ecb); //put failed packet back into queue
    return true;
  }
